...
public double getFullAmount (){ //сумма всех товаров без скидки
   ...
   public double getFullAmountWithDiscount (){ //сумма всех товаров со скидки
        double sum = 0;

        for (int i=0; i<food.length;i++) {

            double itemSum= food[i].getAmount()*food[i].getPriceWithDiscount();
            sum= itemSum+ sum;

        }
        return sum;
    }
    public double getFullAmountVegan (){ //сумма всех товаров без скидки
        double sum = 0;
